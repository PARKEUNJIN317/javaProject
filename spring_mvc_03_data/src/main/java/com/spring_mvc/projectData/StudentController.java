...
public class StudentController {
	...
	//URI를 통한 데이터 전달 받기
	@RequestMapping("/student/studentDataView/{stdNo}")//stdNo pathVariable과 
	public String studentDetailView(@PathVariable String stdNo, Model model) {//매개변수 stdNo는 동일해야함
		System.out.println(stdNo);
		model.addAttribute("stdNo",stdNo);
		return "student/studentResult5";
	}
	
	//서로 다른 타입 데이터의 전송 - 검색관련
	//검색 관련 폼 반환
	...
	//URI를 통한 여러 데이터 전달 받기
		@RequestMapping("/student/studentDataView/{stdNo}/{stdName}/{stdYear}")//stdNo pathVariable과 
	...
	// 2. @RequestParam이용 - 처리함수의 매개변수로 주입
	...
	// 2-1. @RequestParam이용 - 처리함수의 매개변수로 주입
	//form태그 내 input의 name 속성값이 매개변수와 동일하면 @RquestParam을 생략할 수 있다
	//파라미터 명과 
		...
		// 3. command객체 사용 -> 요청처리에 사용되는 객체 command객체라고 함
		// student command객체가 되고 파라미터가 전달된다면 필드명과 같은 이름의 파라미터값이 필드값으로 대입된 객체가 전달됨
		//command 객체를 사용하면 지정된 뷰로 model이 자동 전송됨 -> model에 command 객체가 포함되어서 전송 command 객체명으로 전달
		// command객체 사용시 주의사항 : 파라미터명과 필드명이 동일해야함 다르면 매칭되지 않음(null이 저장됨)
		...
		public String insertStudent4(@ModelAttribute("studentInfo") Student student) { //매개변수명이 아닌 StudentInfo 로 attr의 이름이 설정 
