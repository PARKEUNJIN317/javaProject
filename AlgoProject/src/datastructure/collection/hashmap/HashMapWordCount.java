...
public class HashMapWordCount {
	...
	public static void main(String[] args) {
		...
		//문장을 단어로 분리해서담을 배열 및 word count를 할 HashMap생성
		...
		for(int i=0; i<sentences.length; i++) {
			for(int j=0; j<textArray[i].length; j++) {
				if(!textArray[i][j].isEmpty()) { 
					...
					//map.getOrDefault(key, 기본값) -> key 가 map에 있으면 value를 반환, 없으면 기본값을 반환
					//map의 key가 있을 때, put은 value update
					//기존값 반환받아 1 증가 후 update, 키가 없는 경우 0을 반환받아 1증가 후 put
		...
		//중간 연산은 stream반환 최종연산은 기본값 또는 객체로 결과 반환
		//Hashmap.values() -> stream 처리 -> Integer객체로 반환
		//mapToint(Integer::intValue) :  기본형 int로 변환 처리
		
		int size = wordCount.values().stream().mapToInt(Integer::intValue).sum();
		
		System.out.println("Word counts");
		System.out.println("words : counts : perxentage");
		
		for(Map.Entry<String, Integer>entry : wordCount.entrySet()) {
			System.out.printf("%s : %d : %.2f%%\n", entry.getKey(),entry.getValue(), ((entry.getValue()/size)*100));
		}
		
		//단어 빈도에 따른 통계 출력 : 전체 단어수 : value의 합 /고유단어수 : key의 수 
		...
		// max, min을 위한 비교함수 구현
		//두개의 entry 비교 Comparator 인터페이스 구현(익명객체 사용)
		Comparator<Entry<String, Integer>> comparator = new Comparator<Entry<String, Integer>>(){
			@Override
			public int compare(Entry<String, Integer> e1, Entry<String, Integer> e2) {
				// 비교하려는  value가 Integer고 Integer는 compareTo가 구현되어있음
				return e1.getValue().compareTo(e2.getValue());
			}
		};
		
		// EntrySet을 전달해서 Collection.max/min을 적용하면 Entry타입으로 반환
