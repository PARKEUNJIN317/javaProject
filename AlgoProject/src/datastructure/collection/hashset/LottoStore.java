	...
	public void getLotto(int money) {
		...
		for(int i=1;i<=num; i++) { //로또 장수
			while(lottoSet.size()<6) {//로또 번호 생성
				lottoSet.add((int)(Math.random()*45+1)); // 1~46 범위의 숫자를 set에 저장
			}
			ArrayList<Integer> lottoList = new ArrayList<>(lottoSet); // ArrayList생성자에는 ArrayList로 변환 가능한 객체 전달하면 변환된 객체 반환
