...
public class QueueParking {
	...
	public static void parkingMenu() {
		...
		MENU : while(true) {
			...
			System.out.println("0.주차여부확인 1.주차 2.출차 3.만차 4.종료");
			...
			case "0" :
				...
				if(num<0) {
					System.out.println("입력하신 차량정보는 주차목록에 없습니다.");
				}else {
			...
			case "2" :
				 ...
				 * 0. 출차 고유값 입력
				 * 1. 바로 출차 가능한지 확인
				 * 2. 바로 출차 불가능 하다면 출차할수 없다 출차불가 메세지
				 * 		2-1 주차한지 1시간 이상인지 여부
				 *      2-2 1시간 미만 : 출차불가
				 *      2-3 1시간 이상 : 출차 - 선생차량 dequeue  후 다시 enqueu가 모두 완료된면
				 *      2-4 출차요구 차량 출차(2-3완료되지 않을 시 출차 불가)
				 */
				...
				if(parkChar != 'E') {
					if(Character.toLowerCase(chr1) == parkChar) {//출차진행차량이 맨 앞에 있음
						...
						System.out.println("아직 출차 할 수 없습니다. 주차시간이 1시간이 지난 경우 출차 할 수 있습니다. 주차시간이 1시간 미만이면 0을 입력하세요");
						int time = sc.nextInt();
						sc.nextLine();
						if(time>=1) {//선행차량 이동후 출차
							int num1= parking.contains(Character.toLowerCase(chr1));
							if(num1 >=0) {
								for(int i=1; i<=num1; i++) {
									if(parking.isFull()) {
										System.out.println("주차장에 이동공간이 없어 선행차량 이동이 불가능하여 미리 출차할수 없습니다.");
										continue MENU;
									}else {
