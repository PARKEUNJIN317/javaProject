...
public class MyQueue {
	...
	//큐에 데이터 삽입
	//반환값 수정 : 개발코드(main)에서 enqueue 결과에 따라 다른 처리해야함
	...
	// 현재 주차 확인 중인 차량에 대하여 출차 시 선행 차량 대수 확인용으로 변경
	public int contains(char value) {
		if(isEmpty()) {
			...
			for(int i=front+1; i<=rear; i++) {
				if(queue[i] == value) {
					return i - (front + 1); //i : 현재 확인하려는 원소의 idx - 첫번째 dequeue 대상 원소의 inx
				}
			}
