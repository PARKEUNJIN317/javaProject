...
public class DequeArray {
	public static void main(String[] args) {
		...
		//remove(값) : 해당값이 deque에 있으면 true 반환하고 해당값의 원소를 삭제
		System.out.println("\nremove : " + dq.remove("사과"));
		System.out.println(dq);
		//remove(값) : 해당값이 deque에 없으면 flase반환
		System.out.println("\nremove : " + dq.remove("사과"));
		System.out.println(dq);
		//remove(값) : 해당값이 deque에 있고 동일 값이 여러개 있는 경우 첫번째 값을 반환
		...
		System.out.println(dq.add("포도")); //add성공하면 true반환 용량초과시 Exception
		dq.add("배");
		dq.add("수박");
