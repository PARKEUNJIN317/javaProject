...
public class BinarySearchTree<E> {
	...
	private final Comparator<? super E> comparator; // <? super E> 제너릭타입 E와 E의 모든 부모 클래스 포함
	...
	//type에 비교정의객체가 미구현 타입인 경우에는 Comparator 인터페이스를 구현한 구현객체가 전달되어야 함
	...
	/*
	 * 노드 추가를 위한 addUsingXXX()메소드는 반환타입이 E타입임
	 * 중복 원소를 삽입할 경우 해당 value를 반환하고 삽입이 완료되면 null을 반환함
	 */
	...
	//전달되는 객체에 따라 Comparable 또는 Comparator 중 하나의 사용이 가능하도록 2개 메서드 생성
	...
	private E addUsingComparable(E value) {
		...
		//삽입할 노드가 비교될 수 있도록 변수 생성(add대상의 value가 비교가능하도록 Comparable타입으로 변환)
		...
		if(current == null) {
			...
			size++;
			return null; //노드 추가 완성 시 null반환
		}
		//기존 트리에 노드가 1개이상 있는 경우 방향을 결정
		do {
			//삽입 위치를 유지하는 변수 update
			currentParent = current;
			//새로운 노드의 값과 현재 노드의 값 비교
			compResult = compValue.compareTo(current.value);
			//비교결과에 따라 방향결정, 동일하면 value 그대로 return
		...
		//순회가 완료되어서 삽입해야 할 위치를 찾았다면 value를 노드로 구성
	...
	//Comparator를 이용한 add 메소드 - Comparable이용한 메소드와 비교 메소드만 다르고 다른 절차는 동일함
	...
	private E removeUsingComparable(E value) {
		...
		@SuppressWarnings("unchecked")
		Comparable<? super E> compValue = (Comparable<? super E>)value;
		//삭제 할 node를 찾는 역할을 함, current.value와 compValue가 동일하면 삭제 노드를 찾게됨. 찾으면 break
		...
		if(hasLeft) {//true면 currentParent의 왼쪽 노드가 삭제되었음
