...
public class MergeSort { // 정렬을 위한 공간이 하나 더 필요한 정렬
	...
	private static void merge(int[] a, int left, int mid, int right) {
		...
		int idx = left; //정렬결과를 넣은 idx
		
		//각 부분 리스트의 끝점을 지나지 않는 동안 반복
		//왼쪽 리스트와 오른쪽 리스트들을 비교
		while(l<=mid && r<=right) {
			/*
			 * 왼쪽 부분리스트의 첫번째 원소 값이 더 작으면 임시배열에 왼쪽 부분리스트의 원소를 저장
			 * 정렬이 종료된 원소는 다음 원소로 l++, 임시배열 idx로++
			 */
			if(a[l]<=a[r]) {
				sorted[idx] = a[l];
				idx++;
				 ...
				 * 오른쪽 부분리스트의 첫번째 원소 값이 더 작으면 임시배열에 오른쪽 부분리스트의 원소를 저장
				 * 정렬이 종료된 원소는 다음 원소로 r++, 임시배열 idx로++
				 */
				sorted[idx] = a[r];
				idx++;
				r++;
