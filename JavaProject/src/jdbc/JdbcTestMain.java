...
import java.sql.ResultSet; // select진행 후 반환되는 테이블 결과를 받을 객체

public class JdbcTestMain {

	public static void main(String[] args) {
		// Connection 사용자 정의 클래스 활용 db접속 테스트
		...
		if(con == null) { //db 접속 실패
			...
			rs = stmt.executeQuery(query); // select구문일때 사용하는 메서드, ResultSet반환
			...
			while(rs.next()) {
				// 각메소드가 속성값 1개를 추출, 전체 메소드를 실행하면 1개 레코드의 각 속성 데이터가 변수에 저장됨
