...
public class CRUDTestInsertInput {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//statement 객체는 매번 전처리(형식검사를 진행함 - 완성된 쿼리 구문을 전달해야함)
			...
			////////////////////////////////////////////////////////////////
			// PreparedStatement  객체 복잡한 쿼리 구성의 간결화, 보안측면에서 취약한 쿼리를 배제시킬 수 있음
			// Statement 객체의 하위 인터페이스, 형식 검사를 미리 진행함(컴파일러) - 쿼리 완성 후 형식 검사 진행하지 않음
			//변수 바인딩 진행 : ? 바인딩문자 -> 플레이스홀더 사용, 인수가 많은 쿼리에 적합
			//처음 한번 전처리 후 다음 사용시에는 전처리하지 않는다
			...
			int tmpRes = pstmt.executeUpdate(); //바인딩까지 와내료된 쿼리를 실행
		...
		try {
			...
			System.out.println("도서정보 \t 도서명 \t\t\t\t 저자 \t\t 가격 \t\t 발행일 \t 재고 \t 출판사 번호");
